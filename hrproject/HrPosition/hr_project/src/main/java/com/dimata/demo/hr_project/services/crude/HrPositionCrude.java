...
public class HrPositionCrude {
  ...
  public Flux<HrPosition> createInBatch(Flux<Option> option) {
    return option.flatMap(this::create);
  }
  ...
  private Mono<Option> createRecord(Option option) {
    return Mono
      ...
      .flatMap(
        o -> {
          Mono<HrPosition> savedRecord = dataJurusanDbhandler.create(
            o.getRecord()
          );

          return Mono
            .zip(savedRecord, Mono.just(o))
  ...
  public Mono<HrPosition> updateRecord(Option option) {
    return dataJurusanDbhandler.update(
      option.getRecord(),
      option.getRecord().getId()
    );
  }
